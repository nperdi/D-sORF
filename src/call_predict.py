...
for aLine in multiFaFile:
    ...
    if match:
        ...
        header=aLine.replace(";","_") ##in case there exist a ; in header
...
for aLine in aFile:
    ...
    if (not simulateLength=="") and (not simulateLength=="0"):
        aSeq=aSeq[:startingPos+int(simulateLength)]  ## cut the seq if simmulation 
